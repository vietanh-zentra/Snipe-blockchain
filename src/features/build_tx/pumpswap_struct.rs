...
impl PumpSwapStruct {
    ...
    pub fn get_wsol_ix(
        ...
        // Wrap enough WSOL to match max_quote_in ceiling in get_buy_ix
        // Excess WSOL is returned when close_wsol_ata is called
        let slippage_multiplier = 1.0 + (slippage / 100.0);
        let wsol_amount = buy_amount * 1e9 * slippage_multiplier;
    ...
    pub fn get_buy_ix(
        ...
        // max_quote_in uses slippage multiplier
        // WSOL wrap uses same slippage multiplier
        let expected_tokens: f64 = (sol_amount / token_price) * 10f64.powi(6);
        let base_out: u64 = (expected_tokens * 1.0).max(1.0) as u64;
        let slippage_multiplier = 1.0 + (slippage / 100.0);
        let max_quote_in: u64 = (sol_amount * 10f64.powi(9) * slippage_multiplier) as u64;
